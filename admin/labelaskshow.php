...
if ($action=="del") {
checkadminisdo("label");
$f="../template/".siteskin."/label/askshow/".nostr(trim($_POST["title"])).".txt";
	if (file_exists($f)){
	unlink($f);
	}else{
...
$sql = "select * from zzcms_askclass order by classid asc";
$rs=query($sql);
?>
...
<body>
<div class="admintitle">问答内容标签</div>
...
if (file_exists("../template/".siteskin."/label/askshow")==false){
echo '文件不存在';
}else{		
$dir = opendir("../template/".siteskin."/label/askshow");
       ...
       $rs=query($sql);
		 ...
		 ?>
          ...
          <?php if ($bigclassid<>"empty" && $bigclassid<>""){
			$sql="select * from zzcms_askclass where parentid=" . $bigclassid ." order by classid asc";
			$rs=query($sql);
			while($r=fetch_array($rs)){
			?>
          ...
          <?php   
			...
			?>
        </select> <label><input name="pic[]" type="checkbox" id="pic" value="1" <?php if ($pic==1){ echo " checked";}?>>
        有图片的 </label>
