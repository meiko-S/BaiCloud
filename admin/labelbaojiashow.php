...
if ($action=="del") {
checkadminisdo("label");
$f="../template/".siteskin."/label/baojiashow/".trim($_POST["title"]).".txt";
	if (file_exists($f)){
	unlink($f);
	}else{
...
<body>
<div class="admintitle">报价内容标签</div>
...
if (file_exists("../template/".siteskin."/label/baojiashow")==false){
echo '文件不存在';
}else{			
$dir = opendir("../template/".siteskin."/label/baojiashow");
