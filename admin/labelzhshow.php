...
$f="../template/".siteskin."/label/zhshow/".$title.".txt";
...
if ($action=="del") {
checkadminisdo("label");
$f="../template/".siteskin."/label/zhshow/".nostr(trim($_POST["title"])).".txt";
	if (file_exists($f)){
	unlink($f);
	}else{
...
<body>
<div class="admintitle">展会内容标签</div>
...
if (file_exists("../template/".siteskin."/label/zhshow")==false){
echo '文件不存在';
}else{		
$dir = opendir("../template/".siteskin."/label/zhshow");
...
//读取现有标签中的内容
if ($labelname!=''){
$fp="../template/".siteskin."/label/zhshow/".$labelname;
$f=fopen($fp,"r+");
$fcontent="";
while (!feof($f))
{
